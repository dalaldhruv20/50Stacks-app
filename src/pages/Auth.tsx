...
const phoneSchema = z.string().regex(/^\+?\d{10,15}$/, 'Enter a valid phone number (10-15 digits)');

export default function Auth() {
  const navigate = useNavigate();
  const { user, profile, signInWithGoogle, signInWithEmail, signUpWithEmail, resetPassword, isLoading: authLoading } = useAuth();
  ...
  const [phoneSignupTimer, setPhoneSignupTimer] = useState(0);
  ...
  useEffect(() => {
    if (phoneSignupTimer > 0) {
      const t = setTimeout(() => setPhoneSignupTimer(phoneSignupTimer - 1), 1000);
      return () => clearTimeout(t);
    }
  }, [phoneSignupTimer]);
  ...
  const validatePhone = (value: string) => {
    ...
    try {
      ...
      if (err instanceof z.ZodError) {
        setErrors(prev => ({ ...prev, phone: err.errors[0].message }));
      }
      return false;
    }
  };
  ...
  const handleSendOtp = async (phone: string, context: 'login' | 'signup') => {
    if (!validatePhone(phone)) return;
    
    const setter = context === 'login' ? setIsSendingOtp : setIsSendingOtp;
    setter(true);
  ...
  const handleVerifyOtp = async (phone: string, otp: string, context: 'login' | 'signup') => {
    if (otp.length !== 6) {
    ...
    try {
      ...
      if (data?.success) {
        ...
        // For now, phone verification is done. 
        // Since Supabase auth is email-based, inform user to use email login
        // or implement a custom token-based auth flow
        if (context === 'login') {
          toast.info('Phone verified! Please use your registered email to complete sign in, or sign up if you are new.');
          setOtpSent(false);
          setLoginMethod('email');
        } else {
          toast.info('Phone verified! Please complete your registration with an email and password below.');
          setPhoneSignupOtpSent(false);
  ...
  const renderPhoneOtpForm = (context: 'login' | 'signup') => {
    const phone = context === 'login' ? phoneNumber : phoneNumber;
    const sent = context === 'login' ? otpSent : phoneSignupOtpSent;
    const otp = context === 'login' ? otpValue : phoneSignupOtp;
    const timer = context === 'login' ? otpTimer : phoneSignupTimer;
        ...
        {!sent ? (
          <>
            ...
            <Button
              ...
              className="w-full"
              size="lg"
              disabled={isSendingOtp || (context === 'signup' && phoneSignupName.length < 2)}
              onClick={() => handleSendOtp(phoneNumber, context)}
            >
          ...
          <>
            <div className="text-center space-y-2">
              <p className="text-sm text-muted-foreground">
                Enter the 6-digit OTP sent to <span className="font-medium text-foreground">{phoneNumber}</span>
              </p>
            </div>
            <div className="flex justify-center">
              <InputOTP
                ...
                value={otp}
                onChange={(val) => {
                  if (context === 'login') setOtpValue(val);
                  else setPhoneSignupOtp(val);
                }}
              >
                <InputOTPGroup>
            ...
            <Button
              ...
              onClick={() => handleVerifyOtp(phoneNumber, otp, context)}
            >
            ...
            <div className="flex items-center justify-between text-xs">
              <button
                ...
                onClick={() => {
                  if (context === 'login') { setOtpSent(false); setOtpValue(''); }
                  else { setPhoneSignupOtpSent(false); setPhoneSignupOtp(''); }
                }}
              >
                ← Change number
              </button>
              ...
              <Tabs defaultValue="login" className="w-full">
                ...
                <TabsContent value="signup" className="space-y-4 mt-4">
                  ...
                  <div className="flex gap-2 p-1 rounded-lg bg-secondary/30">
                    <button
                      type="button"
                      onClick={() => { setSignupMethod('email'); setPhoneSignupOtpSent(false); }}
                      className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-md text-xs font-medium transition-colors ${
